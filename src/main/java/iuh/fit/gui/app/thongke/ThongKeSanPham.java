/*
 * @(#) ThongKeSanPham.java 1.0 Nov 6, 2024
...
public class ThongKeSanPham extends JPanel implements ActionListener {
	...
	private SanPhamThongKeService sanPhamThongKeDAO;
    ...
    public static final int COLUMN_INDEX_SOLUONGDABAN = 2;
    public static final int COLUMN_INDEX_TONGTIENBANDUOC = 3;

	public ThongKeSanPham() throws MalformedURLException, NotBoundException, RemoteException {

		setLayout(new BorderLayout());

		sanPhamThongKeDAO = (SanPhamThongKeService) Naming.lookup("rmi://"+ EnvironmentVariable.IP.getValue()+":"+Integer.parseInt(EnvironmentVariable.PORT_SERVER.getValue())+"/sanPhamThongKeService");

		JPanel pnlTitle = new JPanel();
        pnlTitle.setLayout(new MigLayout("", "[]push[][]", ""));
        JLabel lblTitle = new JLabel("Top 5 đồ ăn & uống được mua nhiều nhất");
        ...
        String []header = {"Mã đồ ăn & uống", "Tên đồ ăn & uống", "Số lượng đã bán", "Tổng tiền bán được"};
	...
	private void hienThiBieuDo() throws RemoteException {
        ...
        if (selected != null && selected.contains("/")) {
            ...
            lineDataset = sanPhamThongKeDAO.getDoanhThuSanPhamTheoThangBD(year, month);

        } else if (selected != null) {
            int year = Integer.parseInt(selected);

            lineDataset = sanPhamThongKeDAO.getDoanhThuSanPhamTheoNamBD(year);
        }
	...
	private void loadTableData() throws RemoteException {
        ...
        if (selected != null && selected.contains("/")) {
            ...
            ArrayList<SanPhamThongKe> dsSanPham = sanPhamThongKeDAO.getThongKeSanPhamTheoThang(month, year);

            for (SanPhamThongKe sp : dsSanPham) {
            	String[] row = {sp.getMaSanPham(), sp.getTenSanPham(), Integer.toString(sp.getSoLuongDaBan()), Double.toString(sp.getTongTienBanDuoc())};
                tableModel.addRow(row);
            }
        } else if (selected != null) {
            int year = Integer.parseInt(selected);

            ArrayList<SanPhamThongKe> dsSanPham = sanPhamThongKeDAO.getThongKeSanPhamTheoNam(year);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(o.equals(btnExcel)) {
			String selected = (String) cboThangNam.getSelectedItem();
			ArrayList<SanPhamThongKe> dsSPTK = null;
			String title = "";
			String titleExcel = "";
	        if (selected != null && selected.contains("/")) {
	            String[] parts = selected.split("/");
	            ...
	            titleExcel = "Báo cáo thống kê đồ ăn & uống tháng " + month + " năm " + year;

                try {
                    dsSPTK = sanPhamThongKeDAO.getThongKeSanPhamTheoThang(month, year);
                } catch (RemoteException ex) {
                    throw new RuntimeException(ex);
                }
            } else if (selected != null) {
                ...
                try {
                    dsSPTK = sanPhamThongKeDAO.getThongKeSanPhamTheoNam(year);
                } catch (RemoteException ex) {
                    throw new RuntimeException(ex);
                }
                title = "nam_" +  year;
	            titleExcel = "Báo cáo thống kê đồ ăn & uống năm " + year;
	        }
//	        JFileChooser fileChooser = new JFileChooser();
//	        String excelFilePath = "data/excel/bao_cao_thong_ke_do_an_va_uong_" + title + ".xlsx";
...
//	            pathFile = fileChooser.getSelectedFile().getAbsolutePath();
	        pathFile = "data/excel/bao_cao_thong_ke_do_an_va_uong_" + title + ".xlsx";
	        try {
            	writeExcel(dsSPTK, pathFile, titleExcel);
...
//	        }
	...
	public static void writeExcel(ArrayList<SanPhamThongKe> dsSPTK, String excelFilePath, String title) throws IOException {
        Workbook workbook = getWorkbook(excelFilePath);

        Sheet sheet = workbook.createSheet("Thống kê đồ ăn & uống");
        ...
        rowIndex += 7;
        for (SanPhamThongKe spTK : dsSPTK) {
            Row row = sheet.createRow(rowIndex);
            writeBook(spTK, row, workbook, sheet);
            rowIndex++;
        }
	...
	private static void writeHeader(Sheet sheet, int rowIndex, String title, Workbook workbook) {
		...
		cell4.setCellValue("Top 5 đồ ăn & uống được mua nhiều nhất");
        ...
        cell.setCellValue("Mã đồ ăn & uống");

        cell = row.createCell(COLUMN_INDEX_TENSP);
        cell.setCellStyle(style);
        cell.setCellValue("Tên đồ ăn & uống");

        cell = row.createCell(COLUMN_INDEX_SOLUONGDABAN);
        cell.setCellStyle(style);
        cell.setCellValue("Số lượng đã bán");

        cell = row.createCell(COLUMN_INDEX_TONGTIENBANDUOC);
        cell.setCellStyle(style);
        cell.setCellValue("Tổng tiền bán được");
    }

	private static void writeBook(SanPhamThongKe spTK, Row row, Workbook workbook, Sheet sheet) {
...
//        }
        ...
        Cell cell = row.createCell(COLUMN_INDEX_MASP);
        cell.setCellStyle(style);
        cell.setCellValue(spTK.getMaSanPham());

        cell = row.createCell(COLUMN_INDEX_TENSP);
        cell.setCellStyle(style);
        cell.setCellValue(spTK.getTenSanPham());

        cell = row.createCell(COLUMN_INDEX_SOLUONGDABAN);
        cell.setCellStyle(style);
        cell.setCellValue(spTK.getSoLuongDaBan());

        cell = row.createCell(COLUMN_INDEX_TONGTIENBANDUOC);
        cell.setCellStyle(style);
        cell.setCellValue(spTK.getTongTienBanDuoc());
    }
